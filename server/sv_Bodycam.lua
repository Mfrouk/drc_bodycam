...
SetTimeout(1000, function()
    RegisterUsable('bodycam', function(source)
        ...
        if xPlayer then
            if job then
                TriggerClientEvent('drc_bodycam:show', src, firstname .. ' ' .. lastname, job .. ' RANK: ' .. grade)
            else
                TriggerClientEvent("drc_bodycam:notify", src, "error", locale("error"), locale("invalidjob"))
            end
        end
    end)
end)